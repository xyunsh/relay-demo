...
## 绘制容器
举我们所知,Relay Fragment容器仅仅通过GraphQL fragments申明数据需求.为了实际获取并绘制指定的数据,我们需要用到QueryRenderer组件去绘制根查询以及任何内部包含的fragment容器.请查看[QueryRenderer](query-renderer.md)文档获取更多细节.