...
### 传入参数到Fragment
#### @argumentDefinitions
当我们定义fragment时,你可以使用@rgumentDefinitions指令指定任何参数,以及fragment希望的缺省值.
举个例子,我们使用@argumentDefinitions添加一些参数来重新定义TodoList_list fragment:
```graphql
...
```
任何在@agrumentDefinitions中定义的参数将在fragment域内可见.然而,fragment也可以引用我们定义在根查询中的全局变量.
#### @arguments
