...
- config:
    ...
    - updater: 基于变更从服务端**实际**的返回值用来更新本地内存存储. 如果updater没有提供,缺省情况下, Relay将自动更新变更响应中引用的记录字段. 然而, 如果您进行比较复杂的更新,而不是简单的更新字段(例如删除或者增加项到集合中), 这种情况下,您需要传入updater. 当服务端响应返回时,Relay首先恢复optimisticUpdater或者optimisticReposne中引入的任何修改,然后执行updater. 该函数接受一个store参数, 该参数为Relay Store在内存中的代理. 在改函数中,客户端定义如何通过store实例基于服务端响应来更新本地数据.如何使用store的更多细节,请参阅 [Relay Store API文档](relay-store.md).
